...
* **With no treadmill connected the strip is the call to action instead**, in amber (nothing is broken, there is just one thing left to do): `Connect your treadmill to start a run` — or `Connecting...` / `Reconnecting...`, neither of which is startable either — next to a **Pair** button that opens Pair Devices. It goes back to the two status chips the moment the treadmill is connected.
...
* **The treadmill must be paired before a session can start.** Without it there is no speed to record and nothing to control, so the run would save as a row of zeros. One derived provider (`canStartSessionProvider`) answers "can we start?" for the whole app, and it is true only while the treadmill's connection state is `connected` — a device that is still connecting, or reconnecting after a drop, is not startable, because starting mid-handshake gives the same empty run. **The heart-rate sensor is optional**: it never gates a start, never blocks and never warns. A run with no strap is still a run.
* **This is about starting only.** A session already in progress is never aborted because the treadmill dropped — auto-reconnect and the amber reconnecting chip behave exactly as they always have.
...
* **Starting at all needs the treadmill.** Both modes — structured workout and GPX virtual run — refuse to start while it is not connected. The Library greys out every play button and, because a control that swallows a tap in silence is worse than none, a tap on one still explains itself: a SnackBar reading `Connect your treadmill first — there is nothing to record without it.` with a **Pair** action that opens Pair Devices. That SnackBar replaces the previous one instead of queueing behind it, so three taps say the sentence once rather than for fifteen seconds. `ActiveWorkoutNotifier.startWorkout` returns `Future<bool>` and enforces the same rule defensively — it returns `false` having touched no wakelock, no auto-reconnect, no BLE subscription and no state, and the caller shows that same sentence instead of navigating. A treadmill that has been switched off keeps its GATT link until the supervision timeout, so the gate can still read connected while the radio is dead; that shows up as `startTreadmillListening()` throwing, and that throw refuses the run too rather than being swallowed into a session of zeros. Only the heart-rate top-up and `requestControl` are allowed to fail quietly — HR is optional, and firmware that refuses control still runs the belt by hand. Nothing about this depends on the heart-rate sensor.
